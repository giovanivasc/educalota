...
export const generateStaffBySchoolDoc = async (data: any[]) => {
    try {
        let imgPrefBuf, imgSemedBuf, imgCoordBuf;
        try {
            [imgPrefBuf, imgSemedBuf, imgCoordBuf] = await Promise.all([
        ...
        const separator = new Paragraph({ border: { bottom: { color: "000000", space: 1, style: BorderStyle.SINGLE, size: 6 } }, spacing: { after: 200 } });
        const title = new Paragraph({ alignment: AlignmentType.CENTER, spacing: { before: 200, after: 400 }, children: [new TextRun({ text: `RELATÓRIO - LOTAÇÃO POR ESCOLA`, bold: true, size: 24 })] });

        const tableHeaderRow = new TableRow({
            tableHeader: true,
            children: [
                "Unidade de Ensino", "Cargo / Função", "Nome do Servidor", "Turno"
            ].map((text, i) => new TableCell({
                width: i === 0 || i === 1 ? { size: 25, type: WidthType.PERCENTAGE } : i === 2 ? { size: 35, type: WidthType.PERCENTAGE } : { size: 15, type: WidthType.PERCENTAGE },
                shading: { fill: "2980B9", color: "FFFFFF" },
                verticalAlign: VerticalAlign.CENTER,
                children: [new Paragraph({ alignment: i === 2 ? AlignmentType.LEFT : AlignmentType.CENTER, children: [new TextRun({ text, bold: true, color: "FFFFFF", size: 16 })] })]
            }))
        });
        ...
        data.forEach(row => {
            const isFirstSchool = row.isFirstInSchool;
            const mergeSchool = isFirstSchool ? VerticalMergeType.RESTART : VerticalMergeType.CONTINUE;

            const isFirstRole = row.isFirstInRole;
            const mergeRole = isFirstRole ? VerticalMergeType.RESTART : VerticalMergeType.CONTINUE;

            tableRows.push(new TableRow({
                children: [
                    new TableCell({ verticalMerge: mergeSchool, verticalAlign: VerticalAlign.CENTER, shading: { fill: "F8FAFC" }, children: isFirstSchool ? [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: row.schoolName, bold: true, size: 16 })] })] : [new Paragraph("")] }),
                    new TableCell({ verticalMerge: mergeRole, verticalAlign: VerticalAlign.CENTER, children: isFirstRole ? [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: row.role, size: 16 })] })] : [new Paragraph("")] }),
                    new TableCell({ verticalAlign: VerticalAlign.CENTER, children: [new Paragraph({ alignment: AlignmentType.LEFT, children: [new TextRun({ text: row.staffName, size: 16 })] })] }),
                    new TableCell({ verticalAlign: VerticalAlign.CENTER, children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: row.shift, size: 16 })] })] }),
                ]
            }));
        });
        ...
        const blob = await Packer.toBlob(doc);
        saveAs(blob, `relatorio_lotacao_escola_${d.getTime()}.docx`);
